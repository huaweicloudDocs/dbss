...
3.  执行下列操作之一：
    -   创建新的受保护数据库
        ...
        >-   配置DDM为受保护的数据库，请参见[步骤三：（可选）添加防护的DDM](步骤三-（可选）添加防护的DDM.md)。  
...
创建受保护的数据库之后，为了确保受保护的数据库安全防护策略生效，您还需要将业务侧应用连接的数据库IP地址和端口更改为HexaTier代理的IP地址和端口。详细操作请参见：[步骤四：配置业务侧数据库](步骤四-配置业务侧数据库.md)
